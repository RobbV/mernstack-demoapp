  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id }).then(profile => {
      Post.findById(req.params.id)
        ...
        .catch(err =>
          res.status(404).json({ postnotfound: 'this post doesnt exist' })
        );
    });
  }
);
...
//not working currently not sure why
...
router.post(
  ...
  (req, res) => {
    ...
    //Check Validation
    if (!isValid) {
      //if any erros, send 400 status and errors object
      return res.status(400).json(errors);
    }
    Post.findById(req.params.id)
      .then(post => {
